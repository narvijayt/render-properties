...
<div class="user-profile-hero">
 <!---@if(count($findBanner) >0)
 <div class="vendor_banner user-top-info" @if(count($findBanner) >0)style="background-image: url('{{asset('banner')}}/{{ $findBanner[0]->banner_image }}');" @endif>
  @else
  ...
  <div class="user-top-info">
    <div class="container">
      <div class="row">
       ...
       <div class="col-md-9 user-top-right">
        ...
        <div class="col-md-4">
         ...
         <div class="col-md-8">
            <div class="row">
                @include('pub.partials.users.premium-sidebar')               
            </div>
         </div>
        </div>
        <div class="row">
    ...
    <div class="row">
        ...
        <div class="col-md-4">           
        @if($user->website)
            <i class="fa fa-globe" aria-hidden="true"></i> <a href="{{ real_url($user->website) }}" target="_blank">{{ $user->website }}</a>
        @endif   
        </div>
    </div>
    </div>
    ...
    <!---==================== user-profile-hero END =====================-->
     <div class="container">
        @include('pub.partials.users.premium-profile-info')
            ...
            <form action="{{ route('pub.user.report', $user) }}" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group {{ $errors->has('report') ? 'has-error' : '' }}">
                        ...
                        <textarea
                        type="text" class="form-control" placeholder="Why are you reporting {{ $user->first_name }}'s profile?" name="report" rows="3">{{old('report')}}</textarea>
                        @if($errors->has('report'))
...
<div class="modal fade" id="blockUserModal" tabindex="-1" role="dialog" aria-labelledby="blockUserModal">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="myModalLabel" style="color: #f7991f !important;">Block {{ $user->first_name }}</h4>
            </div>
            <div class="modal-body">
